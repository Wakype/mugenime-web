...
export default async function GenreDetailPage({ params, searchParams }: PageProps) {
  ...
  try {
    ...
    // Jika genre tidak ditemukan atau error
    return notFound();
  }

  const { anime: animeList, pagination } = response;
  const { last_visible_page } = pagination;

  // Format Nama Genre (misal: "action" -> "Action")
  const genreName = genre.charAt(0).toUpperCase() + genre.slice(1).replace(/-/g, " ");

  // Helper Pagination (Konsisten dengan page lain)
  const generatePagination = () => {
    ...
    if (last_visible_page <= maxVisible) {
      ...
      if (currentPage <= 3) {
        ...
        pages.push(1, "...", last_visible_page - 2, last_visible_page - 1, last_visible_page);
      } else {
        pages.push(1, "...", currentPage - 1, currentPage, currentPage + 1, "...", last_visible_page);
      }
    }
    return pages;
  };
      ...
      <div className="container mx-auto px-4 space-y-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4 max-w-2xl">
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm md:text-base leading-relaxed">
                Menampilkan koleksi anime dengan genre <span className="font-bold text-zinc-700 dark:text-zinc-300 capitalize">{genreName}</span>. 
             ...
             <div className="flex flex-col items-end justify-center px-6 py-3 rounded-2xl bg-white/60 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 backdrop-blur-md">
                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Halaman</span>
                <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-black text-indigo-600">{currentPage}</span>
                    <span className="text-sm font-medium text-zinc-400">/ {last_visible_page}</span>
                </div>
            </div>
          </div>
        </div>
        ...
        {animeList && animeList.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-8">
            {animeList.map((anime) => (
              // Gunakan GenreCard di sini
              <GenreCard key={anime.slug} anime={anime} />
            ))}
          </div>
        ) : (
        ...
        <div className="flex items-center justify-center gap-5 pt-8 overflow-x-auto pb-4">
          <Button
            ...
            asChild={pagination.has_previous_page}
            className="h-10 px-4 gap-2"
          >
            {pagination.has_previous_page ? (
              <Link href={`/genre-anime/${genre}?page=${pagination.previous_page}`}>
                <ChevronLeft className="w-4 h-4" /> Sebelumnya
              </Link>
            ) : (
              <span><ChevronLeft className="w-4 h-4" /> Sebelumnya</span>
            )}
          </Button>
          ...
          <div className="flex items-center gap-1">
            {generatePagination().map((p, idx) => {
              if (p === "...") {
                return <span key={`el-${idx}`} className="px-2 text-zinc-400 select-none">...</span>;
              }
              ...
              return (
                ...
                >
                  <Link href={`/genre-anime/${genre}?page=${p}`}>
                    {p}
                  </Link>
                </Button>
              );
          ...
          <Button
            ...
            asChild={pagination.has_next_page}
            className="h-10 px-4 gap-2"
          >
            {pagination.has_next_page ? (
              <Link href={`/genre-anime/${genre}?page=${pagination.next_page}`}>
                Selanjutnya <ChevronRight className="w-4 h-4" />
              </Link>
            ) : (
              <span>Selanjutnya <ChevronRight className="w-4 h-4" /></span>
            )}
          </Button>
        </div>
